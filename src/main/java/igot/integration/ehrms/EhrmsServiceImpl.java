...
import igot.integration.model.ApiRequest;
...
public class EhrmsServiceImpl implements EhrmsService {
    ...
    private String defaultFieldMap;
    ...
    private Map<String, Object> createRequest(Map<String, Object> request, Map<String, Object> profileDetails)
        ...
        Map<String, Object> ehrmsFields = mapper.convertValue(jsonObject.get(Constants.EHRMS_FIELDS), Map.class);
        ...
        for (Entry<String, Object> entry : request.entrySet()) {
            if (!entry.getKey().equalsIgnoreCase(Constants.EMAIL)) {
                if (ehrmsFields.get(entry.getKey()) != null)
                    mappedRequest.put(ehrmsFields.get(entry.getKey()).toString(), entry.getValue());
                else {
        ...
        List<String> nonEhrmsFields = mapper.convertValue(jsonObject.get(Constants.NON_EHRMS_FIELDS), List.class);
        
        Map<String, Object> personalDetails = (Map<String, Object>) profileDetails.get(Constants.PERSONAL_DETAILS);
        for (Entry<String, Object> entry : personalDetails.entrySet()) {
            if (nonEhrmsFields.contains(entry.getKey()))
                mappedRequest.put(Constants.PROFILE_DETAILS+"."+Constants.PERSONAL_DETAILS+"."+entry.getKey(), entry.getValue());
        }
    ...
    // private boolean fieldNotInEhrms(String key, List<String> nonEhrmsFields) throws FileNotFoundException, IOException, ParseException {
    //     List<String> fields = (ArrayList<String>) nonEhrmsFields.get(Constants.FIELDS);
